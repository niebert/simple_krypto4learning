...
function m_setPath(pPath) {
	this.path=pPath;
	this.url_relative = make_relative_path(this.url,pPath);
}
//###################################################################
//###################################################################
...
function convertToHTML(pString)
{
	...
	pString = replaceString(pString,'ˆ','&ouml;');
	pString = replaceString(pString,'¸','&uuml;');
	pString = replaceString(pString,'ƒ','&Auml;');
	pString = replaceString(pString,'÷','&Ouml;');
	pString = replaceString(pString,'‹','&Uuml;');
        pString = replaceString(pString,'ﬂ','&szlig;');
        pString = replaceString(pString,'\n','<br>');
...
//myVar = encodeCGIparam('Hallˆchen & und % proz z');
...
//####### decode a string in a CGI parameter because some characters a not allowed in the URL
{
        ...
        while (vN < pString.length)
	{
		...
		// Character is hexal encoded, i.e. ˆ=%F6
			//mod16 = cc % 16;
...
function convertHTMLtoDOS(pString)
{
   ...
   // problem is caused by the different code tables.
        pString = replaceString(pString,'&auml;','‰');
        pString = replaceString(pString,'&ouml;','ˆ');
        pString = replaceString(pString,'&uuml;','¸');
        pString = replaceString(pString,'&Auml;','ƒ');
        pString = replaceString(pString,'&Ouml;','÷');
        pString = replaceString(pString,'&Uuml;','‹');
        pString = replaceString(pString,'&szlig;','ﬂ');
	return pString
}
...
function normalizeSearchWords(pString)
{
   ...
   // problem is caused by the different code tables.
        ...
        pString = replaceString(pString,'ˆ','oe');
        pString = replaceString(pString,'¸','ue');
        pString = replaceString(pString,'ƒ','ae');
        pString = replaceString(pString,'÷','oe');
        pString = replaceString(pString,'‹','ue');
        pString = replaceString(pString,'ﬂ','ss');
